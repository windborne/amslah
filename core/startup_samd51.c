...
void init_chip() {
	...
	//GCLK->GENCTRL[5].reg = GCLK_GENCTRL_SRC(GCLK_GENCTRL_SRC_XOSC32K);

    //GCLK->GENDIV.reg = GCLK_GENDIV_DIV(48000000/CPU_FREQUENCY) | GCLK_GENDIV_ID(0);
    //GCLK->GENDIV.reg = GCLK_GENDIV_DIV(48000000/PERIPHERAL_FREQUENCY) | GCLK_GENDIV_ID(1);

#ifdef USE_16MHZ_XOSC
    ...
    OSCCTRL->XOSCCTRL[0].bit.IMULT = 5;
    OSCCTRL->XOSCCTRL[0].bit.ENABLE = 1;
    while (!(OSCCTRL->STATUS.bit.XOSCRDY0));

	GCLK->GENCTRL[0].reg = GCLK_GENCTRL_DIV(16000000/CPU_FREQUENCY) | GCLK_GENCTRL_GENEN | GCLK_GENCTRL_RUNSTDBY | GCLK_GENCTRL_SRC_XOSC0;
#else
    GCLK->GENCTRL[0].bit.DIV = 48000000/CPU_FREQUENCY;
/*
#ifndef NOEXTRACLOCKS
    GCLK->GENCTRL[1].bit.DIV = 48000000/PERIPHERAL_FREQUENCY;
#endif
*/
#endif

#ifndef NOEXTRACLOCKS
	GCLK->GENCTRL[3].reg = GCLK_GENCTRL_DIV(32) | GCLK_GENCTRL_GENEN | GCLK_GENCTRL_RUNSTDBY | GCLK_GENCTRL_SRC_OSCULP32K;
    while(GCLK->SYNCBUSY.bit.GENCTRL3);
#endif

    //.bit.SRC = GCLK_GENCTRL_SRC_XOSC0_Val;
	//GCLK->GENCTRL[0].bit.DIV = 16000000/CPU_FREQUENCY;
...
void Reset_Handler(void)
{
	...
	*/
	...
	MCLK->AHBMASK.reg = 0x00C00FFF & (~MCLK_AHBMASK_USB);
	MCLK->APBBMASK.reg = MCLK_APBBMASK_NVMCTRL | MCLK_APBBMASK_PORT | MCLK_APBBMASK_WDT;
	MCLK->APBCMASK.reg = 0;
	MCLK->APBDMASK.reg = 0;
